...
import {
  ...
  Smartphone, Download, ArrowLeft, Quote, BarChart2
} from 'lucide-react';

const BACKEND_URL = import.meta.env.VITE_BACKEND_URL || 'http://localhost:3001';

// ─── Theme Presets — Landing-page aligned ─────────────────────────────────────
const THEME_PRESETS = {
  violet: {
    name: 'Violet',
    pageBg: '#030712',
    ...
    starColor: '#f59e0b',
    featuredBorder: 'rgba(139,92,246,0.3)',
    featuredBg: 'linear-gradient(145deg,#1e1b4b,#111827)',
...
// ─── Helpers ──────────────────────────────────────────────────────────────────
const Stars = ({ rating = 5, color = '#f59e0b' }) => (
  <div style={{ display: 'flex', gap: 2 }}>
    {[1,2,3,4,5].map(i => (
      <Star key={i} size={11} style={{ color: i <= rating ? color : 'rgba(128,128,128,0.25)' }} fill={i <= rating ? color : 'none'} />
    ))}
  </div>
);
...
const Avatar = ({ name = '', photo, size = 32, theme }) => {
  ...
  const palette = [theme?.accent || '#8b5cf6','#7c3aed','#059669','#d97706','#db2777'];
  const bg = palette[(name.charCodeAt(0) || 0) % palette.length];
  if (photo) return <img src={photo} alt={name} style={{ width: size, height: size, borderRadius: '50%', objectFit: 'cover', border: `1px solid ${theme?.cardBorder || 'rgba(255,255,255,0.1)'}`, flexShrink: 0 }} />;
  return <div style={{ width: size, height: size, borderRadius: '50%', background: bg, flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: size * 0.36, fontWeight: 700, color: '#fff', border: `1px solid ${theme?.cardBorder || 'rgba(255,255,255,0.1)'}` }}>{initials || '?'}</div>;
};
...
const VideoCard = ({ t, theme, cfg }) => {
  ...
  return (
    <div style={{ background: theme.cardBg, border: `1px solid ${theme.cardBorder}`, borderRadius: cfg.borderRadius, overflow: 'hidden', transition: 'all 220ms ease', boxShadow: theme.shadow, fontFamily: cfg.fontFamily }}
      ...
      <div style={{ position: 'relative', cursor: 'pointer' }} onClick={toggle}>
        <video ref={ref} src={t.videoUrl || t.VideoURL} style={{ width: '100%', display: 'block', height: 160, objectFit: 'cover' }} onEnded={() => setPlaying(false)} />
        {!playing && (
      ...
      <div style={{ padding: '0.875rem' }}>
        ...
        {(t.Content || t.text) && <p style={{ color: theme.textSecondary, fontSize: cfg.fontSize, lineHeight: 1.65, margin: '8px 0', wordBreak: 'break-word' }}>"{(t.Content || t.text).slice(0, cfg.truncateAt)}{(t.Content || t.text).length > cfg.truncateAt ? '…' : ''}"</p>}
...
const TextCard = ({ t, featured = false, theme, cfg }) => (
  <div style={{ background: featured ? theme.featuredBg : theme.cardBg, border: `1px solid ${featured ? theme.featuredBorder : theme.cardBorder}`, borderRadius: cfg.borderRadius, overflow: 'hidden', transition: 'all 220ms ease', boxShadow: featured ? `0 0 0 1px ${theme.featuredBorder}, ${theme.shadow}` : theme.shadow, fontFamily: cfg.fontFamily, position: 'relative' }}
    onMouseEnter={e => { e.currentTarget.style.borderColor = theme.cardHoverBorder; e.currentTarget.style.transform = 'translateY(-3px)'; }}
    ...
    {featured && <div style={{ position: 'absolute', top: 12, right: 12, fontSize: 9, fontWeight: 700, color: theme.accent, background: theme.accentGlow, border: `1px solid ${theme.featuredBorder}`, borderRadius: 999, padding: '2px 8px', letterSpacing: '0.08em' }}>✦ FEATURED</div>}
    <div style={{ padding: '1rem' }}>
      <Stars rating={t.Rating || t.rating} color={theme.starColor} />
      <p style={{ color: theme.textSecondary, fontSize: cfg.fontSize, lineHeight: 1.7, margin: '10px 0', wordBreak: 'break-word' }}>"{(t.Content || t.text || '').slice(0, cfg.truncateAt)}{(t.Content || t.text || '').length > cfg.truncateAt ? '…' : ''}"</p>
      {(t.imageURL || t.image) && <img src={t.imageURL || t.image} alt="" style={{ width: '100%', borderRadius: 8, marginBottom: 10, height: 110, objectFit: 'cover', border: `1px solid ${theme.cardBorder}` }} />}
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 10, paddingTop: 10, borderTop: `1px solid ${theme.cardBorder}` }}>
        <Avatar name={t.username || t.name} photo={t.UserImageURL || t.photo} theme={theme} />
        <div>
          <p style={{ fontSize: 12, fontWeight: 600, color: theme.textPrimary, margin: 0 }}>{t.username || t.name}</p>
          {t.email && <p style={{ fontSize: 10, color: theme.textMuted, margin: 0 }}>{t.email}</p>}
          <p style={{ fontSize: 10, color: theme.textMuted, margin: 0 }}>{new Date(t.submittedAt || t.createdAt || Date.now()).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}</p>
        </div>
      </div>
    </div>
  </div>
  ...
  return (
    <div style={{ overflow: 'hidden', maskImage: 'linear-gradient(to right, transparent, black 6%, black 94%, transparent)', WebkitMaskImage: 'linear-gradient(to right, transparent, black 6%, black 94%, transparent)' }}>
      <div style={{ display: 'flex', gap: 14, width: 'max-content', animation: `wol-marquee${reverse?'-rev':''} ${speed}s linear infinite` }}>
        {doubled.map((t, i) => <div key={i} style={{ width: 286, flexShrink: 0 }}><Card t={t} index={i} theme={theme} cfg={cfg} /></div>)}
      </div>
    </div>
  );
};
...
const SpotlightLayout = ({ items, theme, cfg }) => {
  ...
  return (
    <div style={{ maxWidth: 680, margin: '0 auto', textAlign: 'center' }}>
      <div style={{ background: theme.featuredBg, border: `1px solid ${theme.featuredBorder}`, borderRadius: cfg.borderRadius, padding: '2.5rem 2rem', position: 'relative', overflow: 'hidden', opacity: fading ? 0 : 1, transform: fading ? 'translateY(8px) scale(0.99)' : 'translateY(0) scale(1)', transition: 'all 260ms cubic-bezier(0.4,0,0.2,1)', boxShadow: theme.shadow, fontFamily: cfg.fontFamily }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'center', marginBottom: 16 }}><Stars rating={t.Rating || t.rating} color={theme.starColor} /></div>
        <p style={{ color: theme.textSecondary, fontSize: cfg.fontSize + 2, lineHeight: 1.8, margin: '0 0 20px', position: 'relative', zIndex: 1 }}>"{(t.Content || t.text || '').slice(0, 300)}{(t.Content || t.text || '').length > 300 ? '…' : ''}"</p>
...
const EmbedModal = ({ spacename, layout, cfg, onClose }) => {
  ...
  return (
    <div onClick={onClose} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.85)', backdropFilter: 'blur(10px)', zIndex: 200, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }}>
      <div onClick={e => e.stopPropagation()} style={{ background: '#111827', border: '1px solid rgba(139,92,246,0.2)', borderRadius: 20, width: '100%', maxWidth: 580, maxHeight: '88vh', overflow: 'hidden', display: 'flex', flexDirection: 'column', boxShadow: '0 32px 80px rgba(0,0,0,0.8)' }}>
        <div style={{ padding: '20px 24px', borderBottom: '1px solid rgba(139,92,246,0.12)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            <div style={{ width: 32, height: 32, borderRadius: 9, background: 'rgba(139,92,246,0.15)', border: '1px solid rgba(139,92,246,0.3)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              <Code2 size={14} style={{ color: '#8b5cf6' }} />
            </div>
            <div>
              <p style={{ margin: 0, color: '#f9fafb', fontWeight: 700, fontSize: 15 }}>Embed Wall of Love</p>
              <p style={{ margin: 0, color: '#4b5563', fontSize: 11 }}>Copy & paste into your website</p>
            </div>
          </div>
          <button onClick={onClose} style={{ width: 30, height: 30, borderRadius: 8, background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', color: '#9ca3af', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <X size={13} />
          </button>
        </div>
        <div style={{ display: 'flex', gap: 3, padding: '12px 24px 0', borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
          {[['iframe','iFrame'],['script','Script']].map(([id, label]) => (
            <button key={id} onClick={() => setTab(id)} style={{ padding: '6px 14px', borderRadius: '8px 8px 0 0', border: `1px solid ${tab===id?'rgba(139,92,246,0.2)':'transparent'}`, background: tab===id?'#111827':'transparent', color: tab===id?'#8b5cf6':'#4b5563', cursor: 'pointer', fontSize: 12, fontWeight: 600, borderBottom: tab===id?'1px solid #111827':'transparent', marginBottom: -1 }}>
              {label}
            </button>
          ))}
        </div>
        <div style={{ flex: 1, overflowY: 'auto', padding: 24 }}>
          <div style={{ background: '#030712', border: '1px solid rgba(139,92,246,0.1)', borderRadius: 12, padding: 16, position: 'relative' }}>
            <pre style={{ margin: 0, fontFamily: "'Space Mono', monospace", fontSize: 11.5, color: '#9ca3af', whiteSpace: 'pre-wrap', wordBreak: 'break-word', lineHeight: 1.8 }}>{snippets[tab]}</pre>
            <button onClick={() => copy(tab)} style={{ position: 'absolute', top: 12, right: 12, display: 'flex', alignItems: 'center', gap: 5, padding: '5px 12px', background: copied===tab?'rgba(16,185,129,0.1)':'rgba(139,92,246,0.1)', border: `1px solid ${copied===tab?'rgba(16,185,129,0.3)':'rgba(139,92,246,0.3)'}`, borderRadius: 8, color: copied===tab?'#10b981':'#8b5cf6', cursor: 'pointer', fontSize: 11, fontWeight: 700 }}>
...
const CustomizerPanel = ({ cfg, setCfg, layout, setLayout, onEmbed, onClose, theme }) => {
  ...
  const labelStyle = { color: '#9ca3af', fontSize: 10, textTransform: 'uppercase', letterSpacing: '0.08em', display: 'block', marginBottom: 7, fontWeight: 600 };
  ...
  const sliderRow = (key, label, min, max, unit = '') => (
    <div style={sectionGap}>
      ...
      <input type="range" min={min} max={max} value={cfg[key]} onChange={e => setCfg(c => ({ ...c, [key]: +e.target.value }))} style={{ width: '100%', accentColor: theme.accent }} />
    </div>
  );

  return (
    <div style={{ width: 264, background: '#030712', borderLeft: '1px solid rgba(139,92,246,0.12)', display: 'flex', flexDirection: 'column', overflow: 'hidden', fontFamily: "'DM Sans', sans-serif" }}>
      <div style={{ padding: '14px 16px', borderBottom: '1px solid rgba(139,92,246,0.12)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 7 }}>
          <Settings2 size={14} style={{ color: theme.accent }} />
          <span style={{ color: '#f9fafb', fontWeight: 700, fontSize: 13 }}>Customize</span>
        </div>
        <div style={{ display: 'flex', gap: 6 }}>
          <button onClick={onEmbed} style={{ display: 'flex', alignItems: 'center', gap: 5, padding: '5px 10px', background: 'rgba(139,92,246,0.12)', border: '1px solid rgba(139,92,246,0.3)', borderRadius: 8, color: '#8b5cf6', cursor: 'pointer', fontSize: 11, fontWeight: 700 }}>
            <Download size={10} /> Embed
          </button>
          <button onClick={onClose} style={{ width: 26, height: 26, borderRadius: 7, background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)', color: '#4b5563', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <X size={12} />
          </button>
        </div>
      </div>
      <div style={{ display: 'flex', borderBottom: '1px solid rgba(139,92,246,0.08)', padding: '8px 10px 0' }}>
        {sections.map(s => (
          <button key={s.id} onClick={() => setActiveSection(s.id)} style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 5, padding: '6px 4px', borderRadius: '7px 7px 0 0', border: 'none', background: activeSection===s.id?'rgba(139,92,246,0.08)':'transparent', color: activeSection===s.id?theme.accent:'#4b5563', cursor: 'pointer', fontSize: 11, fontWeight: activeSection===s.id?700:400, borderBottom: activeSection===s.id?`2px solid ${theme.accent}`:'2px solid transparent', marginBottom: -1 }}>
            {s.icon} {s.label}
          </button>
        ))}
      </div>
        ...
        {activeSection === 'theme' && (
          <div>
            ...
            <div style={sectionGap}>
              ...
              <select value={cfg.fontFamily} onChange={e => setCfg(c => ({ ...c, fontFamily: e.target.value }))} style={{ width: '100%', background: 'rgba(139,92,246,0.06)', border: '1px solid rgba(139,92,246,0.15)', borderRadius: 9, padding: '8px 10px', color: '#f9fafb', fontSize: 12, outline: 'none', appearance: 'none', cursor: 'pointer' }}>
                {FONT_OPTIONS.map(f => <option key={f.value} value={f.value} style={{ background: '#111827' }}>{f.label}</option>)}
              </select>
            </div>
          </div>
        )}
        {activeSection === 'layout' && (
          <div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 7, marginBottom: 20 }}>
              {LAYOUTS.map(l => (
                <button key={l.id} onClick={() => setLayout(l.id)} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 12px', borderRadius: 10, border: `1px solid ${layout===l.id?'rgba(139,92,246,0.3)':'rgba(255,255,255,0.07)'}`, background: layout===l.id?'rgba(139,92,246,0.1)':'rgba(255,255,255,0.02)', cursor: 'pointer', textAlign: 'left', transition: 'all 150ms' }}>
                  <span style={{ fontSize: 16, width: 24, textAlign: 'center', color: layout===l.id?theme.accent:'#4b5563' }}>{l.icon}</span>
                  <div>
                    <p style={{ margin: 0, fontSize: 12, fontWeight: 600, color: layout===l.id?theme.accent:'#f9fafb' }}>{l.label}</p>
                    <p style={{ margin: 0, fontSize: 10, color: '#4b5563' }}>{l.desc}</p>
                  </div>
            ...
            {layout === 'grid' && (
              <div style={sectionGap}>
                ...
                <div style={{ display: 'flex', gap: 7 }}>
                  {[2,3,4].map(n => (
                    <button key={n} onClick={() => setCfg(c => ({ ...c, columns: n }))} style={{ flex: 1, padding: '8px', borderRadius: 8, border: `1px solid ${cfg.columns===n?'rgba(139,92,246,0.4)':'rgba(255,255,255,0.07)'}`, background: cfg.columns===n?'rgba(139,92,246,0.12)':'transparent', color: cfg.columns===n?'#8b5cf6':'#4b5563', cursor: 'pointer', fontSize: 13, fontWeight: 700 }}>{n}</button>
                  ))}
                </div>
              </div>
            )}
            {[{ key: 'featuredFirst', label: 'Feature First Card' }, { key: 'showTitle', label: 'Show Page Title' }].map(({ key, label }) => (
...
export default function TestimonialWall() {
  ...
  const theme = THEME_PRESETS[cfg.themeKey] || THEME_PRESETS.violet;
  ...
  useEffect(() => {
    ...
    if (search.trim()) { const q = search.toLowerCase(); list = list.filter(t => [t.username, t.name, t.Content, t.text, t.email].join(' ').toLowerCase().includes(q)); }
  ...
  const renderLayout = () => {
    if (loading) return (
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', minHeight: 320, gap: 14 }}>
        <div style={{ width: 36, height: 36, borderRadius: '50%', border: `3px solid rgba(139,92,246,0.2)`, borderTopColor: '#8b5cf6', animation: 'wol-spin 0.8s linear infinite' }} />
    ...
    <>
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        ::-webkit-scrollbar { width: 4px; } ::-webkit-scrollbar-track { background: transparent; } ::-webkit-scrollbar-thumb { background: rgba(139,92,246,0.2); border-radius: 2px; }
      ...
      <div style={{ minHeight: '100vh', background: theme.pageBg, color: theme.textPrimary, fontFamily: "'DM Sans', sans-serif", display: 'flex', flexDirection: 'column', transition: 'background 300ms' }}>
        ...
        {/* Top Bar */}
        <div style={{ borderBottom: `1px solid ${isLight ? '#e8e8e8' : 'rgba(139,92,246,0.12)'}`, background: isLight ? 'rgba(255,255,255,0.9)' : 'rgba(3,7,18,0.88)', backdropFilter: 'blur(14px)', position: 'sticky', top: 0, zIndex: 50 }}>
          <div style={{ maxWidth: 1380, margin: '0 auto', padding: '0.8rem 1.5rem', display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap', justifyContent: 'space-between' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
              <button onClick={() => navigate(`/space/${spacename}`)} style={{ display: 'flex', alignItems: 'center', gap: 5, background: 'none', border: 'none', color: theme.textMuted, cursor: 'pointer', fontSize: 12, padding: 0 }}>
                <ArrowLeft size={13} /> Back
              </button>
              <div style={{ width: 1, height: 18, background: isLight ? '#e8e8e8' : 'rgba(139,92,246,0.15)' }} />
              <div style={{ display: 'flex', alignItems: 'center', gap: 7 }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 7 }}>
              <button onClick={() => setIsMobilePreview(m => !m)} style={{ display: 'flex', alignItems: 'center', gap: 5, padding: '6px 11px', borderRadius: 9, border: `1px solid ${isMobilePreview ? 'rgba(139,92,246,0.4)' : isLight ? '#e8e8e8' : 'rgba(255,255,255,0.08)'}`, background: isMobilePreview ? 'rgba(139,92,246,0.12)' : 'transparent', color: isMobilePreview ? '#8b5cf6' : theme.textMuted, fontSize: 11, cursor: 'pointer', fontWeight: 600, transition: 'all 150ms' }}>
                {isMobilePreview ? <Smartphone size={12} /> : <Monitor size={12} />}
                {isMobilePreview ? 'Mobile' : 'Desktop'}
              </button>
              <button onClick={() => setShowCustomizer(c => !c)} style={{ display: 'flex', alignItems: 'center', gap: 5, padding: '6px 11px', borderRadius: 9, border: `1px solid ${showCustomizer ? 'rgba(139,92,246,0.4)' : isLight ? '#e8e8e8' : 'rgba(255,255,255,0.08)'}`, background: showCustomizer ? 'rgba(139,92,246,0.12)' : 'transparent', color: showCustomizer ? '#8b5cf6' : theme.textSecondary, fontSize: 11, cursor: 'pointer', fontWeight: 600, transition: 'all 150ms' }}>
                <Settings2 size={12} /> Customize
              </button>
              <button onClick={() => setShowEmbed(true)} style={{ display: 'flex', alignItems: 'center', gap: 5, padding: '6px 11px', borderRadius: 9, border: `1px solid ${isLight ? '#e8e8e8' : 'rgba(255,255,255,0.08)'}`, background: 'transparent', color: theme.textSecondary, fontSize: 11, cursor: 'pointer', fontWeight: 600, transition: 'all 150ms' }}>
        ...
        <div style={{ flex: 1, display: 'flex', overflow: 'hidden' }}>
          <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'auto', transition: 'all 300ms' }}>
            {cfg.showTitle && (
              <div style={{ textAlign: 'center', padding: '2.25rem 1.5rem 1.5rem', background: `radial-gradient(ellipse 55% 50% at 50% 0%, rgba(139,92,246,0.1) 0%, transparent 70%)`, animation: 'wol-fadeup 0.5s ease both' }}>
                <div style={{ display: 'inline-flex', alignItems: 'center', gap: 6, padding: '4px 14px', background: 'rgba(139,92,246,0.1)', border: '1px solid rgba(139,92,246,0.2)', borderRadius: 999, marginBottom: 12 }}>
                  <Sparkles size={11} style={{ color: '#8b5cf6' }} />
                  <span style={{ fontSize: 11, fontWeight: 600, color: '#8b5cf6', letterSpacing: '0.1em', textTransform: 'uppercase' }}>Wall of Love</span>
                </div>
                <h1 style={{ fontFamily: "'Syne', sans-serif", fontWeight: 800, fontSize: 'clamp(1.4rem, 3.5vw, 2.2rem)', color: theme.textPrimary, marginBottom: 6 }}>{spaceInfo?.space_name || spacename}</h1>
                ...
                <div style={{ display: 'inline-flex', alignItems: 'center', gap: 14, background: theme.cardBg, border: `1px solid ${theme.cardBorder}`, borderRadius: 999, padding: '6px 18px', fontSize: 12 }}>
                  <span style={{ color: theme.textSecondary }}><span style={{ color: theme.textPrimary, fontWeight: 700 }}>{stats.total}</span> reviews</span>
            ...
            {/* Controls bar */}
            <div style={{ borderBottom: `1px solid ${isLight ? '#e8e8e8' : 'rgba(139,92,246,0.08)'}`, background: isLight ? 'rgba(255,255,255,0.6)' : 'rgba(3,7,18,0.4)', backdropFilter: 'blur(8px)', padding: '0.7rem 1.5rem', position: 'sticky', top: 53, zIndex: 40 }}>
              <div style={{ maxWidth: 1380, margin: '0 auto', display: 'flex', gap: 10, alignItems: 'center', flexWrap: 'wrap' }}>
                <div style={{ display: 'flex', gap: 4 }}>
                  {LAYOUTS.map(l => (
                    <button key={l.id} onClick={() => setLayout(l.id)} title={l.label} style={{ width: 30, height: 30, borderRadius: 8, border: `1px solid ${layout===l.id?'rgba(139,92,246,0.4)':isLight?'#e8e8e8':'rgba(255,255,255,0.07)'}`, background: layout===l.id?'rgba(139,92,246,0.12)':'transparent', color: layout===l.id?'#8b5cf6':theme.textMuted, cursor: 'pointer', fontSize: 14, display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'all 150ms', fontWeight: 700 }}>
                      {l.icon}
                    </button>
                  ))}
                </div>
                <div style={{ width: 1, height: 18, background: isLight ? '#e8e8e8' : 'rgba(139,92,246,0.12)' }} />
                <div style={{ display: 'flex', gap: 5 }}>
                  {[['all','All'],['text','Text'],['video','Video'],['liked','❤️ Loved']].map(([f, label]) => (
                    <button key={f} onClick={() => setFilter(f)} style={{ padding: '4px 11px', borderRadius: 999, fontSize: 11, fontWeight: filter===f?700:500, border: `1px solid ${filter===f?'rgba(139,92,246,0.4)':isLight?'#e8e8e8':'rgba(255,255,255,0.07)'}`, background: filter===f?'rgba(139,92,246,0.12)':'transparent', color: filter===f?'#8b5cf6':theme.textMuted, cursor: 'pointer', transition: 'all 150ms', whiteSpace: 'nowrap' }}>
                      {label}
                    </button>
                  ))}
                </div>
                <div style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: 8 }}>
                  <select value={minRating} onChange={e => setMinRating(+e.target.value)} style={{ background: theme.cardBg, border: `1px solid ${theme.cardBorder}`, borderRadius: 8, color: theme.textSecondary, fontSize: 11, padding: '5px 8px', outline: 'none', cursor: 'pointer' }}>
                  ...
                  <div style={{ position: 'relative' }}>
                    ...
                    <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search…" style={{ background: theme.cardBg, border: `1px solid ${theme.cardBorder}`, borderRadius: 9, color: theme.textPrimary, fontSize: 11, padding: '6px 10px 6px 28px', outline: 'none', width: 150 }} />
            ...
            <div style={{ textAlign: 'center', padding: '1rem', color: theme.textMuted, fontSize: 11, borderTop: `1px solid ${theme.cardBorder}`, marginTop: 'auto' }}>
              Powered by <span style={{ color: '#8b5cf6', fontWeight: 700 }}>TestiQra</span>
            </div>
          </div>

          {showCustomizer && (
            <CustomizerPanel cfg={cfg} setCfg={setCfg} layout={layout} setLayout={setLayout} onEmbed={() => setShowEmbed(true)} onClose={() => setShowCustomizer(false)} theme={theme} />
          )}
        </div>
      </div>
